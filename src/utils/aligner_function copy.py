...
# Note: These imports assume the protenix package is available in the path
...
def _estimate_sublattice_radius(lattice: Lattice, voxel_size_original: float) -> float:
    ...
    # Fallback: estimate from dimensions using the corrected formula
    # Reverse: radius = 0.5*voxel_size + radius_voxels*voxel_size
    # So: radius_voxels = (sublattice_dimension - 1) / 2
    # And: radius = 0.5*voxel_size + ((dim - 1) / 2) * voxel_size = (dim / 2) * voxel_size
...
def _downsample_volumes_and_lattice(
    ...
    # Estimate sublattice radius (in Angstroms - physical units, not voxels)
    # NOTE: The sublattice radius stays the same in Angstroms (e.g., 5 Å), but the number of voxels
    # needed to cover that radius decreases when voxel size increases. The Lattice class automatically
    # converts: num_voxels = ceil((radius_A / voxel_size_A) - 0.5). So if radius=5Å and voxel_size doubles,
    # the number of voxels halves, which is correct - we still cover the same physical space.
...
def esp_se3_align_ensemble(
    ...
    # Pre-alignment: Simple symmetric approach
    # 1. Compute each member's centroid
    # 2. Pick a target centroid (member 0's centroid for consistency)
    # 3. Translate each member by (target_centroid - member_centroid) to align centroids
    # 4. Then rotate each member to align with member 0's orientation
    with torch.no_grad():
        ...
        # Step 1: Translate all members to align centroids
        # Translation for each member: (target_centroid - member_centroid)
        translations = target_centroid[None, :] - member_centroids  # [B_ens, 3]
        
        # Apply translations
        coords_translated = coords_ens + translations[:, None, :]  # [B_ens, N, 3]
        
        # Step 2: Rotate all members to align with member 0's orientation
        # Center all members at target_centroid for rotation alignment
        ...
        for i in range(B_ensembles):
            # All members (including 0) go through Kabsch for symmetry
    ...
    # Apply pre-alignment: rotation + translation back to target_centroid
    # All members end up at target_centroid with aligned orientations
    ...
    # DEBUG: Check pre-alignment quality (for identical structures, all should be at same position)
    if B_ensembles > 1:
        pre_align_rmsds = []
    ...
    # CRITICAL: Use ens_centroid directly, NOT the average of coords_pre!
    # Averaging coords_pre introduces tiny numerical errors (even 0.00002 RMSD differences)
    # that create asymmetric offsets: (coords_pre[i] - centroid) will be different for each member
    # All members are pre-aligned TO ens_centroid, so use it directly for perfect symmetry
    ...
    if use_so3_grid:
        ...
        if so3_grid_resolution is not None:
            ...
            # Auto-select: find the largest grid resolution that fits within n_random
            # SO3 grid sizes: resol=0->72, resol=1->576, resol=2->4608, ...
            best_resol = 0
    ...
    # Simple initialization: Since coords_pre are already pre-aligned, all ensemble members
    # get the same rotation and translation for each hypothesis.
    # Use repeat() instead of expand() to allocate separate memory (expand creates shared gradient views)
    ...
    for it in range(1, steps + 1):
        ...
        # DEBUG: Check parameter divergence (same as local_tests) - always print for debugging
        if B_ensembles > 1 and it in [1, 5, 10, 20]:
    ...
    with torch.no_grad():
        ...
        # Convert to origin-centered rotation: x_out = x_in @ R.T + T_origin
        # From centroid-centered: x_out = (x_in - c) @ R.T + c + T
        #                              = x_in @ R.T + (c - c @ R.T + T)
        # So: T_origin = c - c @ R.T + T
        # Using einsum for batched c @ R.T: einsum('j,eij->ei', c, R) = c @ R[e].T for each e
        # (Note: 'eij' means R[e,i,j], so sum_j c[j]*R[e,i,j] = (c @ R[e].T)[i])
        R_comp = R_best  # [B_ens, 3, 3]
        c_times_RT = torch.einsum('j,eij->ei', centroid, R_best)  # [B_ens, 3]
        T_comp = centroid - c_times_RT + T_best  # [B_ens, 3] (centroid broadcasts to [1,3])
        ...
        if compute_rmsd:
            ...
            # Always print final results (not just when verbose)
