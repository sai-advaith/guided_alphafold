...
def get_all_atom_coordinates(pdb_file):
...
class EPRLossFunction(AbstractLossFunction):
    def __init__(self, constraints_file, rotamers_statistics_file, rotamers_structures_folder, device, atom_array, batch_size, sample_points=1000):
    ...
    def save_tensor_to_pdb(self,coords: torch.Tensor, atom_template, path: str):
        """
        Save a (N, 3) coordinate tensor to PDB using biotite, keeping atom metadata.
        """
        assert coords.shape == (atom_template.array_length(), 3)
        coords_np = coords.detach().cpu().numpy()

        atom_array = atom_template.copy()
        # atom_array.coord = coords_np
        atom_array.chain_id = np.array([id_[0] for id_ in atom_array.chain_id])
    ...
    def plot_location_distribution(self, probability, dists, constraint, i, time):
        os.makedirs("epr_distributions", exist_ok=True)
        probs = probability.cpu().numpy()
        dists = dists.detach().cpu().numpy().squeeze()

        plt.figure(figsize=(max(6, 10), 4))  # Adjust width dynamically
        plt.bar(dists, probs)
        plt.xlabel("O1 Location")
        plt.ylabel("Probability")
        plt.title(f"Distribution of distances for Residue {constraint}")
        plt.tight_layout()
        plt.savefig(f"epr_distributions/time_{time*200:.0f}_epr_distribution_{constraint}_struct_{i}.png")
        plt.close()
            ...
            if updated_probs[key[0]].sum() != 0 and updated_probs[key[1]].sum() != 0:
                ...
                for j in range(len(key0_probs)):
                    if key0_probs[j].sum() != 0 and key1_probs[j].sum() != 0:
                        ...
                        if time*200 > 195:
                            self.plot_location_distribution(prob0[indices0]*prob1[indices1], dist, key, j, time)
            if len(key_dist) > 0:
